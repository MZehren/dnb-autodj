...
class TransitionProfile:
    def __init__(self, len_dbeats, volume_profile, low_profile, high_profile):
		...
		"""
        if not (
            (volume_profile[0][0] == low_profile[0][0] == high_profile[0][0] == 0)
            and (
                volume_profile[len(volume_profile) - 1][0]
                == low_profile[len(low_profile) - 1][0]
                == high_profile[len(high_profile) - 1][0]
                == len_dbeats
            )
        ):
