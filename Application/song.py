...
class Song:
    ...
    def getSegmentType(self, dbeat):
        ...
        raise Exception(
            "Invalid downbeat " + str(dbeat) + ", should be between " + str(self.segment_indices[0]) + ", " + str(self.segment_indices[-1])
        )
    ...
    def annotate(self):
        ...
        if not self.hasSpectralContrastSummaryAnnot():
            ...
            writeBinaryAnnotFile(
                self.dir_,
                self.title,
                ANNOT_SPECTRALCONTRAST_PREFIX,
                np.concatenate((specCtrstAvgs, specValleyAvgs, specCtrstDeltas, specValleyDeltas)),
            )
        ...
        if not self.hasSingingVoiceAnnot():
            ...
            is_singing = np.array(singing_model.decision_function(singing_scaler.transform(X)), dtype="single")
    ...
    def loadAnnotSegments_fixNegativeStart(self):
        ...
        if self.segment_indices[0] < 0:
            ...
            self.onset_curve = np.append(
                np.zeros((1, self.songBeginPadding / 512)), self.onset_curve
            )  # 512 is hop size for OD curve calculation
    ...
    def calculateSongThemeDescriptor(self):
        ...
        self.song_theme_descriptor = theme_pca.transform(theme_scaler.transform(features.astype("single").reshape((1, -1)))).astype(
            "single"
        )
