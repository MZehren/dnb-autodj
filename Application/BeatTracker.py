...
class BeatTracker:
    ...
    def __init__(self, minBpm=99.0, maxBpm=148.0, stepBpm=0.01, FRAME_SIZE=1024, HOP_SIZE=512, SAMPLE_RATE=44100.0):
        self.minBpm = minBpm
        self.maxBpm = maxBpm
    ...
    def run(self, audio):
        ...
        for bpm in valid_bpms:
            frames = (np.round(np.arange(0, np.size(novelty_autocorr), numFramesPerBeat(bpm))).astype("int"))[
                :-1
        ...
        for phase in valid_phases:
            ...
            frames = (np.round(np.arange(phase_frames, np.size(novelty_hwr), numFramesPerBeat(bpm))).astype("int"))[
                :-1
            ]  # Discard last value to prevent reading beyond array (last value rounded up for example)
...
if __name__ == "__main__":
    ...
    audioMarked = onsetMarker(audio / 2.0)
