...
import numpy as np
import sys, os
from DownbeatTracker import featureLoudness, featureMFCC, featureOnsetIntegral, featureOnsetIntegralCsd, featureOnsetIntegralHfc
...
class DownbeatTracker:
    ...
    def track(self, song):
        ...
        for i, j, row in zip(range(len(probas)), np.array(range(len(probas))) % 4, probas):
            ...
            permuted_row[4 - j :] = row[
                :j
            ]  # element i of permuted_row (i = 0,1,2,3) corresponds to the TRAJECTORY over the song starting with downbeat 0, 1, 2, 3
